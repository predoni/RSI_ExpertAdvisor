...
class MyPositions
{
   public:
      ...
      static ulong GetTicket_PositionsActiveForType(ENUM_POSITION_TYPE type, string algorithm);
      static ulong GetTicket_OrdersActiveForType(ENUM_ORDER_TYPE typeIn, string algorithm);
};
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
static ulong MyPositions::GetTicket_PositionsActiveForType(ENUM_POSITION_TYPE typeIn, string algorithm)
{
   int totalPositions = PositionsTotal();
   for (int positionIndex=0; positionIndex < totalPositions; positionIndex++) {
      ulong positionTicket = PositionGetTicket(positionIndex);
...
//+------------------------------------------------------------------+
static ulong MyPositions::GetTicket_OrdersActiveForType(ENUM_ORDER_TYPE typeIn, string algorithm)
{
   int total = OrdersTotal();
   for (int index=0; index < total; index++) {
      ulong ticket = OrderGetTicket(index);
